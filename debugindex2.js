...
async function securityLock(){
  if (process.env.SUP3V_S3CV3T_STRING){
    if (process.env.J0HNNYK1N_SUP3V_S3CV3T_STRING){
      if (process.env.J0HNNYK1N_SUP3V_S3CV3T_STRING == process.env.SUP3V_S3CV3T_STRING){
        console.log('Success!')
...
async function deriveKey(password, salt) {
  const hash = await argon2.hash(password, {
    ...
    timeCost: 2,
    memoryCost: 1024,
    parallelism: AvailableParallelism, // To dynamically work based on available paralleism
...
async function deriveSalt(password, salt) {
  ...
  const hash2 = await argon2.hash(password, {
    salt,
    type: argon2.argon2id,
    hashLength: 16, // 32 bytes (256 bits)
    timeCost: 2,
    memoryCost: 1024,
...
const ANCESTOR_MASTER_KEY = crypto.randomBytes(32); // Replace with a securely generated key
let AMK = ANCESTOR_MASTER_KEY;

let AES_STRING = "AES_STRING_OF_TEXT";
...
if (getAES() === "AES_STRING_OF_TEXT"){
  ...
  setAES(Buffer.from(process.env.ENCRYPTION_KEY, 'hex'), Buffer.from(process.env.ENCRYPTION_SALT, 'hex'));
}else{//We load key from vault and use in runetime!
  console.log('Hey it exists in context already!');
  process.env.ENCRYPTION_KEY = Buffer.from(getAES(), 'hex'); //Specify to retreive back to a key buffer in memory to use
}
...
async function saveMasterKey(masterKey, ancestorMasterKey) {
  ...
  const LockOutSalt = await deriveSalt(process.env.EXTRA_SECURITY_KEY, extraSalty);
  console.log('Made it here on saving master Key also!');
  console.log('LockOutSalt is :', LockOutSalt.toString('hex'));
  const LockOutKey = await deriveKey(process.env.J0HNNYK1N_SUP3V_S3CV3T_STRING, LockOutSalt);
  ...
  const data = {
    lockoutKey: encryptLockOutKey,  //Properly Derive the key with a salt!
    ancestorKey: encryptSkelly,    // Save the AMK
    salt2: extraSalty.toString('hex'),   //To Truly be secure we also encrypt our salt as well this is the final completion to the product!
    salt3: salt2.toString('hex'), //Is LockOutSalt is the true key answer to it all! //Is our true salt to get and retain memory for the decryption of skeleton ancestorKey. Which doesn't leak the actual salt!
    salt4: salt4.toString('hex'),
    salt5: salt5.toString('base64'), //So we have restructured to utilize a LockoutKey design this key is generated using a process.env string! this is the key used to encrypt skeleton ancestor key 
    salt6: salt6.toString('hex'),  //Doing so makes a 3 way lock solution with lock out design! allowing proper re creation of the lock chain as well incase of errors!
    ...
    salt7: iv.toString('hex'),//Before we where saving the encrypted data with the master key in context still this is bad practice!
...
async function loadMasterKey() {
  ...
  if (process.env.USE_DB === 'true'){
    ...
    console.log('process:', process.env.EXTRA_SECURITY_KEY, '\n Salt:', Buffer.from(data.salt3, 'hex'));
    const extraSaltyLockoutSalt = await deriveSalt(process.env.EXTRA_SECURITY_KEY, Buffer.from(data.salt2, 'hex'));
    console.log('Extra salty lockoutSalt hex:', extraSaltyLockoutSalt.toString());
    const extraSaltyLockoutKey = await deriveKey(process.env.J0HNNYK1N_SUP3V_S3CV3T_STRING, extraSaltyLockoutSalt);
    ...
    const masterKeyHex = await decryptData(
...
async function handleEncryptionDecryption() {
  ...
  const rotatePassword = async () => {
    ...
    try {
      ...
      //Rotate Skeleton Key
      AMK = generatePassword();
      await saveMasterKey(masterKey, AMK); // Save the newly generated master key
    }
...
//        console.log('Where before error lol!');
        ...
        await encrypt(masterKey, AES_STRING);
      }
      }catch(err){
    ...
    await saveMasterKey(newPassword, AMK);
